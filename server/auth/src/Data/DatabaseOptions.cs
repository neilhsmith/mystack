...
public sealed class DatabaseOptions
{
    ...
    // clients, someone able to sign in — without hand-work. Safe to leave on because writes
    // happen only on real drift, and every account is config-declared, so no environment
    // receives anything it didn't declare (architecture §3.4). Off is the escape hatch for an
    // organisation managing clients out of band.
