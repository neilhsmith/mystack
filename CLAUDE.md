...
- Request DTO validation via **FluentValidation**, **run inside the service layer** — validation is bound to the service contract, not the HTTP transport. One `AbstractValidator<T>` per request DTO, co-located with the DTOs in `<Feature>Dtos.cs`. The service injects the validators and runs them at the top of each write method; failures come back as `List<Error.Validation>` and [`ErrorResults.ToProblem`](apps/api/src/Api/Http/ErrorResults.cs) shapes them into the standard RFC 9457 `application/problem+json` envelope (same wire shape clients have always seen). The upshot: any caller — endpoint handler, background job, internal service, integration test — gets validation for free; there is no "endpoint filter validated it before calling me" assumption. Field constraints live in a nested `public static class Constraints` on the entity (`Post.Constraints.MaxTitleLength`, `Post.Constraints.MaxContentLength`) and are referenced by EF fluent config, FluentValidation, and the OpenAPI spec — one source of truth per entity.
