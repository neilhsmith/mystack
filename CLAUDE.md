...
- RFC 7232 ETag support on single-resource endpoints via `src/Api/Data/ETag.cs` (strong tag from `ITimestamped.UpdatedAt`) and `src/Api/Data/ConditionalRequest.cs` (304 on `If-None-Match`; 428/412 on missing/stale `If-Match` for writes). Wired into Posts GET/POST/PUT/DELETE.
- Request DTO validation via **FluentValidation** — one validator class per DTO in the feature folder (`CreatePostRequestValidator`, `UpdatePostRequestValidator`). The generic `ValidationEndpointFilter<T>` in `src/Api/Validation/` runs the validator before the handler and short-circuits with RFC 9457 `application/problem+json` (via `Results.ValidationProblem`). Validators reference shared scalar limits as `public const`s on the entity (e.g. `Post.MaxTitleLength = 200`) so EF fluent config, FluentValidation, and OpenAPI all read from one source of truth.
...
- **OpenAPI spec at `/openapi/v1.json`** (`Microsoft.AspNetCore.OpenApi`, ASP.NET 10 native). `FluentValidationSchemaTransformer` reflects validator rules (`required`, `maxLength`, `minLength`, `pattern`) into the schema so downstream consumers see the same constraints the runtime enforces. `OpenApiSpecTests` is the regression guard.
