...
| `Instance:Name` | `mystack` | Names this running stack instance; validated at boot as a non-empty `[A-Za-z0-9_-]` token. The application and antiforgery cookies carry it (`.MyStack.Auth.<name>`, `.MyStack.Antiforgery.<name>`) because browsers scope cookies to the host and ignore the port — and with per-instance data-protection rings, two instances sharing a cookie name on one host don't merely collide, each evicts the other's session. Override it for every stack running beside the default one. |
