...
**The model, decided first:** the apps redirect to auth's hosted pages — the industry's
hosted-login pattern, load-bearing for this architecture (credentials exist on one origin, SSO
rides auth's session cookie, an app XSS cannot keylog a password field that isn't there).
Embedding the forms in web/admin was considered and rejected: every route to it is the password
grant, third-party-cookie breakage, or re-implementing the account surface per app. So the
pages are designed as what hosted login pages are: a **centered card on a quiet background** —
logo wordmark, form, links, one-line footer, no app chrome. Trust reads as quiet.

**Lands:** every rendered page designed rather than scaffolded — sign in, register, forgot
password, reset password, confirm email, resend confirmation, change password, device
verification, root, error, signed-out, access-denied, the end-session confirmation; there is no
consent page (architecture D17). The design language is **default shadcn (Base UI flavor)**,
themed later — these pages adopt the stock look so the future design-system pass restyles
everything by editing tokens.

- **The shared theme is a file, not a duplication.** The JS workspace is bootstrapped
  (pnpm-workspace + `packages/ui`) by importing the shadcn components wholesale — **no
  Storybook, no component tests yet**, both parked below — because the import is what mints the
  canonical stylesheet: the `@theme` token file (palette, radius, typography, light + dark)
  every frontend app and auth share. This supersedes the earlier "consciously duplicated
  tokens" note: tokens are design *data*, and duplication would drift.
- **Auth consumes the token source, never a built stylesheet.** Tailwind output is
  content-scanned per app — web's `app.css` can't style pages it never scanned — so
  `server/auth` runs its own **standalone-CLI** build: an input css that `@import`s the shared
  tokens and `@source`s the Pages, emitting one committed `wwwroot/app.css`. The CLI is a
  build-time binary — no Node at auth's runtime; the committed output is regenerated by script
  and gate-checked for freshness, csharpier-style. Generated, never hand-edited.
- **One `_Layout.cshtml` replaces thirteen standalone documents** — the card shell, a text
  wordmark, per-page `<title>`s. The shadcn recipes (button, input, card, label, alert) are
  expressed as utility classes in the markup, mirroring what `packages/ui` renders, so the
  server pages and the future React apps read as one product.
- **Light and dark ship together**, switched by `prefers-color-scheme`; the tokens carry both
  palettes and a `.dark` class override stays possible for the parked theme-cookie integration.
- **CSP widens exactly as planned**: `style-src 'self'` (plus `img-src 'self'` only if the
  wordmark becomes an asset), never `'unsafe-inline'`. Fonts stay the system stack (no
  `font-src`). The pages stay JS-free — no password-reveal toggle; password managers own that,
  fed by correct `autocomplete` tokens.
- **Accessibility is a deliverable, not a vibe**: WCAG 2.2 AA. Every input labelled;
  `autocomplete` (`email`, `current-password`, `new-password`, `one-time-code` for the device
  user code); invalid postbacks render an error summary that receives focus and anchor-links
  each field; visible `:focus-visible` rings; AA contrast in both palettes; `lang`; unique
  titles. Verified by a manual axe + keyboard + screen-reader pass over all thirteen pages,
  recorded in the PR — the automated guard is parked below.
- `docs/auth.md` finalized — the read-through-and-tighten pass, including the pages prose.

**Shape:** two PRs. First `feat(ui): bootstrap the JS workspace and the shared shadcn theme`
(pnpm-workspace, `packages/ui` with the imported components, the token stylesheet). Then
`feat(auth): design the rendered pages` (the Tailwind pipeline, the layout, the thirteen
pages, the CSP widening, the a11y pass, the auth.md finalization).

**Proves:** the full suite green — markup-sensitive tests updated, never weakened; the
committed stylesheet regenerates byte-identical in CI; the accessibility pass documented; and
the track's closing exercise — an end-to-end walkthrough of the whole project until it is
genuinely understood rather than merely working.

> React components are still **not** reused on these pages (§2's rule, and a credentials host
> must not depend on a JS bundle). The share is the token file and the visual recipes — data
> and convention, not code.
...
- **Third-party clients** — D17's boundary: no consent screen, implicit-consent clients only,
  ...
  being first-party.
- **Theme cookie for the auth pages** — when web/admin grow their theme feature: the BFF sets a
  `theme` cookie on the shared parent domain (cookies ignore ports, so plain `localhost` works
  in dev), and auth — server-rendered — reads it and stamps `class="dark"` over the
  `prefers-color-scheme` default, so a choice made in an app follows the user to sign-in.
  Trigger: the first app theme toggle.
- **Component-library tooling** — Storybook, Vitest unit tests and Storybook interaction tests
  over `packages/ui`. Trigger: the first React app consuming the package; until then the
  components sit imported-but-unexercised as the token source (15b's decision).
- **Automated accessibility guard** — Playwright + axe sweeping the thirteen rendered pages,
  wired into CI. Trigger: the JS e2e infrastructure arriving with the first app; 15b's manual
  pass carries the bar until then.
- **Client-aware sign-in greeting** — the authorize request names the client, so the sign-in
  page could render "Sign in to MyStack Admin" from the client's display name. Small, cosmetic,
  parked until the second browser client exists to distinguish.
