...
**Landed**, as the two planned PRs. First `feat(ui): bootstrap the JS workspace and the shared
shadcn theme`: pnpm-workspace + Prettier + the gate's JavaScript half, and `packages/ui` with
the shadcn (Base UI flavor) components imported wholesale — **no Storybook, no component tests
yet**, both parked below — because the import is what mints the canonical token stylesheet
(`theme.css`) every frontend app and auth share. Then `feat(auth): design the rendered pages`:
all thirteen pages — sign in, register, forgot/reset password, confirm/resend confirmation,
change password, device verification, root, error, signed-out, access-denied, the end-session
confirmation; no consent page (architecture D17) — designed as **default shadcn**, so the
future design-system pass restyles everything by editing tokens.

- **The theme deviates from stock shadcn in one deliberate way.** Instead of `:root` + `.dark`
  palette blocks (which need JavaScript to toggle a class), every token carries both palettes
  in one `light-dark()` value and `color-scheme` picks the side: the JS-free auth pages follow
  the OS preference natively, a `.light`/`.dark` class on `<html>` still forces a theme (the
  parked theme-cookie hook), and there is no second palette block to forget when retheming.
  The `dark:` variant mirrors the same rules with two arms.
- **Auth consumes the token source, never a built stylesheet** — Tailwind output is
  content-scanned per app — via `Styles/app.css`: `@import` the shared tokens by relative
  path, `@source` the Pages, compile with `pnpm build:css` into the committed
  `wwwroot/app.css`, freshness-checked by the gate, csharpier-style. One simplification from
  the plan: the compiler is the pnpm-pinned `@tailwindcss/cli` rather than a downloaded
  standalone binary — the workspace bootstrap made the version-pinned CLI free, and the
  property that mattered (no Node at auth's *runtime*) holds either way.
- **One `Pages/Shared/_Layout.cshtml` replaces thirteen standalone documents** — centered
...
- **CSP widened exactly as planned**: `style-src 'self'` on the pages policy only, never
  ...
  password managers own that, fed by correct `autocomplete` tokens.
- **The a11y bar**: every input labelled with the right `autocomplete` token; invalid
  postbacks render an error summary that receives focus (`tabindex="-1" autofocus`,
  `role="alert"`) and anchor-links each field error; `aria-invalid` + `aria-describedby` on
  fields; `:focus-visible` rings from the recipes; AA contrast in both palettes; `lang`;
  unique titles. The scaffold's stray empty validation-summary bullet is gone with the
  scaffold. Axe run over all thirteen pages locally; the human keyboard + screen-reader pass
  rides the walkthrough below; the automated guard is parked.
- `docs/auth.md` finalized — the pages prose rewritten as built, the "functional now, designed
  later" hedges removed.

**Still open, the track's closing exercise:** an end-to-end walkthrough of the whole project
until it is genuinely understood rather than merely working — architecture §7's checkbox waits
for it.
