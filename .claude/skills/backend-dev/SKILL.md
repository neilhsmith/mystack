...
- [ ] **Endpoint filter**: every POST / PUT / PATCH handler with a request body must be decorated `.AddEndpointFilter<ValidationEndpointFilter<TRequest>>()`. The filter runs **before** any handler logic — including 404 lookups — so a malformed body never burns a DB roundtrip.
...
The combined effect on endpoint shape:

```csharp
// GET /<resource>/{id} — middleware adds the ETag and handles If-None-Match → 304.
var entity = await db.<DbSet>.AsNoTracking().FirstOrDefaultAsync(p => p.Id == id, ct);
return entity is null ? TypedResults.NotFound() : TypedResults.Ok(mapper.ToResponse(entity));

// PUT /<resource>/{id} — exception handler catches concurrency races and emits 409.
var entity = await db.<DbSet>.FirstOrDefaultAsync(p => p.Id == id, ct);
if (entity is null) return TypedResults.NotFound();
mapper.Apply(request, entity);
await db.SaveChangesAsync(ct);
return TypedResults.Ok(mapper.ToResponse(entity));
```
...
2. Resource lookup (404) — return early if the row's gone.
3. Apply the change, `await db.SaveChangesAsync(ct)`. A concurrent writer's race surfaces as `DbUpdateConcurrencyException` → 409 from the exception handler. No try/catch in the handler.
